...
export function loadConfig(): AppConfig {
  ...
  const bffApiBaseUrl =
    env.BFF_API_BASE_URL?.trim() || `${bffPublicUrl.replace(/\/$/, "")}`;
  ...
  const sessionSecret = requireEnv(
    "THIRD_SESSION_SECRET",
    "third-session-secret-change-me",
  );

  const sessionCookieName = env.THIRD_SESSION_COOKIE_NAME?.trim() || "third_sid";
  const sessionCookieSecure =
    (env.THIRD_SESSION_COOKIE_SECURE ?? "false").toLowerCase() === "true" ||
